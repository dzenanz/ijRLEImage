...
\authoraddress{$^{1}$dzenan.zukic@kitware.com\\
               $^{2}$matt.mccormick@kitware.com\\
               $^{3}$pauly2@mail.med.upenn.edu}
...
\doxygen{LabelMap}\cite{Lehmann2007} uses run-length encoding to store objects
...
\code{itk::RLEImage} uses a simpler data structure, which makes it more suited
for on-the-fly editing.
Thus \code{itk::RLEImage} and \doxygen{LabelMap} have quite different goals.
...
ITK-SNAP employed before, and not significantly slower than  \textit{pure ITK} RoI filter.
